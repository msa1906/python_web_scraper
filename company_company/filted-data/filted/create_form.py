...
for filename in os.listdir(directory):
	if filename.endswith(".csv") and filename.startswith("2017-"):
		print(filename)
			...
			if len(row) > 5:
				...
				for i in range(4, len(row) ):
					for j in range(i + 1, len(row) ):
						...
						print('com1 ' + str(film[row[i]]) + ' ' + row[i] + 'com2 '+ str(film[row[j]]) + ' '+ row[j]+'counter' + str(Matrix[film[row[i]]][film[row[j]]]))
